...
*/
  ...
  this.getEventsByTarget = function (options) {
    ...
    var params = {
      start: GSC.CybozuConnect.myUtility.formatXSDDateTime(options.start, false),
      end: GSC.CybozuConnect.myUtility.formatXSDDateTime(options.end, false)
    };
    if (options.userId) {
  ...
  this.modifyRepeatEvent = function (event, modifyType, baseDate, modifyDate) {
    ...
    if (modifyType == "this" && !GSC.CybozuConnect.myUtility.equalDate(baseDate, modifyDate)) {
        if (event.allDay) {
          event.start = event.end = modifyDate;
        } else {
          var xsdModifyDate = GSC.CybozuConnect.myUtility.formatXSDDate(modifyDate);
          event.start = GSC.CybozuConnect.myUtility.parseXSDDateTime(xsdModifyDate + "T" + event.repeatInfo.start_time);
          if (event.repeatInfo.end_time) {
              event.end = GSC.CybozuConnect.myUtility.parseXSDDateTime(xsdModifyDate + "T" + event.repeatInfo.end_time);
          }
        }
        var eventId = event.id; // save
        event.event_type = "normal";
        var visStart = new Date(modifyDate);
        var visEnd = GSC.CybozuConnect.myUtility.incDate(visStart);
      ...
      } else if (modifyType == "after" && GSC.CybozuConnect.myUtility.compareDate(baseDate, modifyDate) > 0) {
        var eventId = event.id; // save
        var visStart = new Date(modifyDate);
        var visEnd = GSC.CybozuConnect.myUtility.incDate(visStart);
        var events = this.addEvent(event, visStart, visEnd);
        ...
        event.repeatInfo.start_date = GSC.CybozuConnect.myUtility.formatXSDDate(baseDate);
      }
      ...
      if (modifyType == "this" || modifyType == "after") {
        operation.date = GSC.CybozuConnect.myUtility.formatXSDDate(baseDate);
      }
  ...
  this.removeEventFromRepeatEvent = function (eventId, type, date) {
    ...
    if (type == "this" || type == "after") {
      params.operation.date = GSC.CybozuConnect.myUtility.formatXSDDate(date);
    }
  ...
  this.addFollowToRepeatEvent = function (event, date, followText) {
    ...
    var events = this.addFollowsToRepeatEvent({ event_id: event_id, date: GSC.CybozuConnect.myUtility.formatXSDDate(date), content: followText });
    if ($.isArray(events) && events.length) return events[0];
  ...
  function createWhen(event) {
    if (event.allDay) {
      var date = { start: GSC.CybozuConnect.myUtility.formatXSDDate(event.start) };
      if (event.end) {
        date.end = GSC.CybozuConnect.myUtility.formatXSDDate(event.end);
      ...
      if (event.end) {
        datetime.end = GSC.CybozuConnect.myUtility.formatISO8601(event.end, true); // not xsd:datetime
      }
  ...
  function createEventFromResponse($event, start, end) {
    ...
    if (e.allDay) {
      ...
      e.start = GSC.CybozuConnect.myUtility.parseISO8601(datetime.attr("start"), true);
      if (!e.start_only) {
        e.end = GSC.CybozuConnect.myUtility.parseISO8601(datetime.attr("end"), true);
      }
    }
  ...
  function createRepeatedEventsFromReponse($event, $repeat_info, start, end) {
    ...
    for (var i = 0; i < dates.length; i++) {
      var date = dates[i];
      var xsdDate = GSC.CybozuConnect.myUtility.formatXSDDate(date);
      ...
      if (e.allDay) {
        ...
        e.start = GSC.CybozuConnect.myUtility.parseXSDDateTime(xsdDate + "T" + e.repeatInfo.start_time);
        if (!e.start_only && e.repeatInfo.end_time) {
            e.end = GSC.CybozuConnect.myUtility.parseXSDDateTime(xsdDate + "T" + e.repeatInfo.end_time);
        }
  ...
  function getRepeatDates($repeat_info, repeatInfo, start, end) {
    ...
    var startDate = GSC.CybozuConnect.myUtility.parseXSDDate(repeatInfo.start_date);
    var endDate = GSC.CybozuConnect.myUtility.parseXSDDate(repeatInfo.end_date);
    ...
    $repeat_info.find("exclusive_datetime").each(function () {
      var datetime = GSC.CybozuConnect.myUtility.parseXSDDateTime($(this).attr("start"));
      if (datetime) {
    ...
    if (type == "lastweek" || (type == "month" && day == 0)) {
      var lastDate = GSC.CybozuConnect.myUtility.getLastDate(date);
    ...
    while (date.getTime() <= endDate.getTime()) {
      ...
      date = GSC.CybozuConnect.myUtility.incDate(date);
    }
