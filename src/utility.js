...
var myUtility  = {
  ...
  xmlAttributeEscape: function (text) {
    ...
    return GSC.CybozuConnect.myUtility.htmlEscape(text).replace(/\r/g, "&#xD;").replace(/\n/g, "&#xA;");
  },
  ...
  formatISO8601: function (value, utc) {
    ...
    return GSC.CybozuConnect.myUtility.formatXSDDateTime(value, utc) + "Z";
  },
  ...
  parseXSDDateTime: function (text) {
    ...
    var date = GSC.CybozuConnect.myUtility.parseXSDDate(vals[0]);
    if (!date) return null;
    vals = vals[1].split("+");
    vals = vals[0].split(":");
    if (vals.length < 3) return null;
  ...
  getLastDay: function (year, month) {
    ...
    return GSC.CybozuConnect.myUtility.getLastDate(new Date(year, month, 1)).getDate();
  },
