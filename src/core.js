...
//var appFunction = function (url) {
  ...
  var CybozuType = { Garoon: "Garoon", GaroonCloud: "GaroonCloud" };
  ...
  } else if (_cybozuURL.indexOf(".cybozu.com/g/") >= 0 || _cybozuURL.indexOf(".cybozu-dev.com/g/") >= 0 ) {
  ...
  this.exec = function (service, method, params, alertIfError, debug) {
    ...
    time += 1000;
    var expires = GSC.CybozuConnect.myUtility.formatISO8601(time, false);

    // request
    method = GSC.CybozuConnect.myUtility.xmlEscape(method);
...
<UsernameToken>\
  <Username>' + GSC.CybozuConnect.myUtility.xmlEscape(cybozuUsername) + '</Username>\
  <Password>' + GSC.CybozuConnect.myUtility.xmlEscape(cybozuPassword) + '</Password>\
</UsernameToken>\
</Security>';
    }
...
<' + method + ' xmlns="http://wsdl.cybozu.co.jp/base/2008">';
    ...
    */
    ...
    if (req.status == 200 || req.responseText) {
      ...
      if (req.responseXml) {
        ...
        dom = GSC.CybozuConnect.myUtility.xmlDom(req.responseText);
      }
      if (dom != null) {
        var $dom = $(dom);
        ...
        if (req.responseText.indexOf("GRN_CBPAPI_63006") >= 0) {
          this.error = "ライセンスの有効期限を過ぎています。";
        } else {
  ...
  // private functions
  function _makeParametersXml(name, child) {
    ...
    for (var key in child) {
      ...
      if (value == null) {
        ...
        xml += _makeParametersXml(key, value);
        ...
        for (var i = 0; i < value.length; i++) {
          ...
          xml += _makeParametersXml(key, value[i]);
        }
        ...
        xml += GSC.CybozuConnect.myUtility.htmlEscape(value);
        break;
        ...
        if (type == "string") {
          if (value.indexOf("\n") >= 0) {
            xml += " " + key + '="' + GSC.CybozuConnect.myUtility.xmlAttributeEscape(value) + '"';
          } else {
            xml += " " + key + '="' + GSC.CybozuConnect.myUtility.htmlEscape(value) + '"';
          }
